...
38. **Tech Spec** — Document generation engine, `/nemesis` Phase 3. Creates Razorpay Tech Spec Google Docs (16-section TECH_SPEC_TEMPLATE, matching [Optimizer Offers Phase 1](https://docs.google.com/document/d/1Pgljgc-9H35Bdl7k5LVgM68zMLwSu0fzg2ZyzYLCwP8) reference doc). Section 8 (Final Approach) uses DEEP sub-sections (8.2.x.y level) with API JSON examples, code diffs, math proofs, DB schemas, approach comparisons. Razorpay-first tool priority. Diagrams via **Canva MCP first** (professional quality), Mermaid fallback for structural diagrams.
...
43. **Tech Spec @Slash verification** — Tech Spec runs 3-5 @Slash fact-check queries during doc generation (Step 0.5, before section content generation). Queries target Razorpay standards (tech spec template, NFRs, monitoring, testing requirements). Results enrich sections 9/11/13 and are cited in section 16.1. Not optional — verified facts > assumed facts.
44. **TECH_SPEC_TEMPLATE has deep sub-sections** — 16 sections matching the Razorpay reference doc format ([Optimizer Offers Phase 1](https://docs.google.com/document/d/1Pgljgc-9H35Bdl7k5LVgM68zMLwSu0fzg2ZyzYLCwP8)). Section 8 (Final Approach) has 10 sub-sections (8.1-8.10) with dynamic per-flow deep sub-sections (8.2.x.y level). Other sections have 2-6 sub-sections each. Total ~45 sub-sections. Tech Spec uses TECH_SPEC_TEMPLATE for feature specs; SOLUTION_DOC_TEMPLATE (15 sections, 35 sub-sections) is reserved for bug-fix analysis docs only.
...
48. **Two doc templates** — `TECH_SPEC_TEMPLATE` (16 sections, for feature design specs) and `SOLUTION_DOC_TEMPLATE` (15 sections, for bug fix analysis). Tech Spec defaults to TECH_SPEC_TEMPLATE. Use `--template solution` only for targeted fix analysis docs. Both templates have rich sub-sections. The TECH_SPEC_TEMPLATE matches the Razorpay standard reference doc format.
...
59. **Eager expert init** — `brain init` initializes all 46 project experts to L2 eagerly. Each expert deep-reads routing patterns, middleware, config mechanisms, key structs, and API endpoints. Idempotent — re-running skips already-initialized experts.
